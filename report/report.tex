...
Python scikit-image(\texttt{skimage.transform.probabilistic_hough_line}) implementation of \textit{Probabilistic Hough Transform} was used to detect straight lines in medial axis skeleton. In this method straight line is represented by the segment perpendicular to it, parametrised by a pair (\ro, \theta), where \ro is a length of segment, and \theta is an angle. Parameter's plane of (\ro, \theta) is divided into discrete grid (by parameter frequency  value bins), and the array representing this grid contains number of pixel close to the respective line \cite{Kiryati}.
